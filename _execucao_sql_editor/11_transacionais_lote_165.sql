...
-- Conciliação - PIX RECEBIDO - LUCIANO DE SOUSA FONT...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - VIVIANI DOS SANTOS LI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - 50.728.767 JUSTINO MA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - AMALIA RANGEL...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - MARCOS LUIS GUSMAO DA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - PATRICIA PACIFICO DA ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CAROLINE MAIOLINI MIZ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - CASA DO XAVIE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - NICOLAS COUTI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'FUNDO DE GARANTIA DO TEMPO DE SERVIÇO (competência 08/2024)',
        'FUNDO DE GARANTIA DO TEMPO DE SERVIÇO (competência 08/2024)',
        '2025-10-18'::date,
        747.66,
        1,
...
-- VALE ADIANTAMENTO: Carolina Rodrigues da Silva (au...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - JOAQUIM ANDRE DE MELO...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- CARTÃO FIDELIDADE: Lojas > Alecio dos Santos > PIX...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- RESCISÃO: Zaine de Lima Siqueira - Acordo 1/14 > P...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'RESCISÃO: Zaine de Lima Siqueira - Acordo 1/14 > PIX: 275.783.178-06 (CPF)',
        'RESCISÃO: Zaine de Lima Siqueira - Acordo 1/14 > PIX: 275.783.178-06 (CPF)',
        '2024-09-20'::date,
        1250.0,
        1,
...
-- COFINS: Simples Nacional 08/2024...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: ACERTO FRANQUIA > Parcela 30/40 > PIX: 39....
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- VALE TRANSPORTE: Kathleen dos Santos Campos (ref. ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - DENIS APARECI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - VANIA SANTOS ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - JODENILSON SANTOS SAM...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - ULISSES ROBER...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- PROMOÇÃO QUEM INDICA AMIGO É!: OS 924 - Maria Zule...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - STEPHANIE ALV...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- CARTÃO INDICAÇÃO: Indicou, ganhou! - lojas > Andre...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - HENRIQUE OLIVEIRA RAM...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - GRAZIELE CUSTODIO MAR...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ARMARINHOS FERNANDO LTDA: Folha sulfite > Mike Rod...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - LUAN FRANÇA DE OLIVEI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - GIVANILDO PEDRO DAS D...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
