...
-- Conciliação - PIX RECEBIDO - CP :10573521-ADRIANA ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :10573521-NEEMIAS ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO INTERNO - 00019 0198622...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :90400888-CELSO BO...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :00360305-GIOVANO ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-ROBERTA ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'ENTRADA DE COMPRA: Compra da loja - Óticas Taty Mello: Parcela 6/11',
        'ENTRADA DE COMPRA: Compra da loja - Óticas Taty Mello: Parcela 6/11',
        '2023-04-24'::date,
        2090.0,
        1,
...
-- Conciliação - PIX - RECEBIDO - 17/04 18:29 0000462...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO - 17/04 13:43 1210924...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60746948-LUIZ CAR...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO - 18/04 16:50 0001683...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO - 18/04 13:02 0002650...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SUPERMERCADO NAGUMO: Água sanitária e álcool líqui...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO19/04 14:47 0004976997...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :00000000-ICARO MA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60746948-ISABEL D...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - TARIFA PACOTE DE SERVIЗOSCOBRANЗA RE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- FREELANCER: Paulo Sérgio Bueno Junior (ref. 05/04 ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- OPTOMETRISTA: Eduardo 1° Quinz. Abril  > PIX: 226....
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO20/04 11:09 0003306097...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO - 20/04 16:50 0000034...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'ENTRADA DE COMPRA: Compra da loja - Óticas Taty Mello: Parcela 6/6',
        'ENTRADA DE COMPRA: Compra da loja - Óticas Taty Mello: Parcela 6/6',
        '2023-01-24'::date,
        2400.0,
        1,
...
-- Conciliação - PIX RECEBIDO - CP :90400888-ADRIANO ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-BÁRBARA ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO - 24/04 13:43 1210924...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO21/04 12:16 0005692957...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO - 21/04 12:45 0003870...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- PORTO SEGURO: Seguro fiança - Parcela 3/12...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO24/04 16:40 1952270000...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
