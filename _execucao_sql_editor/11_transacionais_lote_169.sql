...
-- Conciliação - QR CODE PIX RECEBIDO - MAYME SANTOS ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- INSTITUTO VISÃO:  1° Quinz. Outubro > PIX: 35.128....
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- COMPRA DE PAPEL HIGIÊNICO - Rosangela > PIX: 116.3...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - CARTÃO DE DÉBITO - ASSAI ATACADISTA ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: ACERTO FRANQUIA > Parcela 34/40 > PIX: 39....
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - VANESSA GOMES...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'FUNDO DE GARANTIA DO TEMPO DE SERVIÇO (competência 09/2024)',
        'FUNDO DE GARANTIA DO TEMPO DE SERVIÇO (competência 09/2024)',
        '2025-10-18'::date,
        747.28,
        1,
...
-- GAINEN CERTIFICADORA > E-CNPJ GIOVANNA COMERCIO > ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX ENVIADO - JOSE TOSI NETO...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: ACERTO FRANQUIA > Parcela 4/54 > PIX: 39.4...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - RODRIGO BISPO...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - DIRCE GABRIEL...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - ERICA TAMIRIS...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - TATIANE DOS SANTOS OL...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'Conciliação - PIX RECEBIDO - TATIANE DOS SANTOS OLIVEIRA',
        'Conciliação - PIX RECEBIDO - TATIANE DOS SANTOS OLIVEIRA',
        '2024-10-20'::date,
        1398.0,
        1,
...
-- Conciliação - TRANSFERÊNCIA ENVIADA - GILBERTO DE ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- COFINS: Simples Nacional 09/2024...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - KAUE VICTOR F...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - JAQUELINE FERREIRA DA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - ROBERT LAHUER...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
