...
-- Conciliação - PIX RECEBIDO - ALINE SANTIAGO PEREIR...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- HONORÁRIOS: Bruna Giovanna Cardoso > Parcela 4/8 >...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SUPERMERCADO NAGUMO: Itens para a loja > Nicolas C...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - LAURA DIAS FERNANDES ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - ELAINE GOMES DE MORAE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: Bruna Giovanna Cardoso > Parcela 2/12 > PI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SUPERMERCADO NAGUMO: Itens para o escritório > Mik...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - SERGIO ROBERTO DE SOU...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- PREMIAÇÃO - META BATIDA: Rosangela Aparecida Dejan...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- VALE TRANSPORTE: Andressa Bezerra da Silva > PIX: ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- MONTAGEM: Jhonathan Nunes dos Santos > PIX: jhol20...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - MARISA DE ARAUJO BATI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- MANUTENÇÃO DAS PORTAS - Suzano I: Luis da Silva > ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- PREMIAÇÃO - META BATIDA: Wevilly de Souza Fernande...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: ACERTO FRANQUIA > Parcela 24/40 > PIX: 39....
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - PEDRO HENRIQU...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - ANDRESSA BEZERRA DA S...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - OZENI PEREIRA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'Conciliação - QR CODE PIX RECEBIDO - OZENI PEREIRA DA CRUZ SILVA',
        'Conciliação - QR CODE PIX RECEBIDO - OZENI PEREIRA DA CRUZ SILVA',
        '2024-08-12'::date,
        1650.0,
        1,
...
-- Conciliação - QR CODE PIX RECEBIDO - SANDRA CRISTI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: VISION PRIME - Parcela 13/49 > PIX: 34.130...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - DIEGO PEREZ D...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ESTORNO CLIENTE FRANQUIA: Gilvanilda Ventura dos S...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - JORGE MAYCON AMARAL M...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - LAZARO APARECIDO MACH...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
