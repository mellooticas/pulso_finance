...
-- Lote 49/86 (500 lançamentos)

-- Conciliação - PIX RECEBIDO - CP :59285411-LUCILENE...
DO $$
DECLARE
...
-- Conciliação - PIX RECEBIDO INTERNO - 00019 0233101...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-ARIANI D...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- VALE TRANSPORTE: Ariani Dias Fernandes Cardozo (di...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60746948-ADRIANA ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'Conciliação - PIX RECEBIDO - CP :60746948-ADRIANA APARECIDA PIRES ARAUJO',
        'Conciliação - PIX RECEBIDO - CP :60746948-ADRIANA APARECIDA PIRES ARAUJO',
        '2023-07-18'::date,
        1100.0,
        1,
...
-- Conciliação - PIX RECEBIDO - CP :31872495-ARIELI C...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- BRASLAB PRODUTOS OTICOS EIRELI > R$ 1.327,22...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- 20/06/2023 - BRASLAB PRODUTOS OTICOS EIRELI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-DAIANE D...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SALÁRIO: Isabela Santos Silva Alves (ref. os dias ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :00360305-LUIZ FER...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ALUGUEL CLÍNICA - MAUÁ (IPTU) > Banco: Bradesco I ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SALÁRIO: Bianca Raquel da Silva Santos (ref. os di...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO INTERNO - 00019 0117387...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO INTERNO - 00019 0146526...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :90400888-ELIANE C...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO INTERNO - 00019 0216761...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :10573521-NATALICE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60746948-ELOISA A...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :00360305-EVANY DO...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-JÚLIA AL...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- PORTO SEGURO: Seguro fiança - Parcela 6/12...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SALÁRIO: Felipe Arruda Lellis Bentoglio (divulgado...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-SUELLEN ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- VILA PERUS LAR & CONSTRUCAO: Desentupidor para san...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- VT referente os dias trabalhados, período 24/07-31...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SALÁRIO: Thais de Sousa Aliona (divulgadora, ref. ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :90400888-DANIELLE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- PAGBANK - PIX RECEBIDO - CP :08561701-GILBERTO DE ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'PAGBANK - PIX RECEBIDO - CP :08561701-GILBERTO DE CAMARGO SILVA JUNIOR',
        'PAGBANK - PIX RECEBIDO - CP :08561701-GILBERTO DE CAMARGO SILVA JUNIOR',
        '2023-07-25'::date,
        2371.42,
        1,
...
-- Conciliação - PIX RECEBIDO - CP :31872495-RUTE FER...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'Conciliação - PIX RECEBIDO - CP :31872495-RUTE FERREIRA',
        'Conciliação - PIX RECEBIDO - CP :31872495-RUTE FERREIRA',
        '2023-07-25'::date,
        1690.0,
        1,
