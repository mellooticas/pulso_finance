...
-- Conciliação - QR CODE PIX RECEBIDO - LAURA DIAS FE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - JAQUELINE DOS...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- BONIFICAÇÃO: Nicolly Mello (ref. ida para a casa d...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- FRANQUIA > PIX: 52.597.549/0001-09 (CNPJ) - (MAUÁ)...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- FRANQUIA > PIX: 52.597.549/0001-09 (CNPJ) - (SUZAN...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - EDUARDA FERNA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ESTORNO CLIENTE: Maria Nazareth de Oliveira  > PIX...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - DOUGLAS LOPES...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - LUCIVANE DANTAS DE AR...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - LILIAN APARECIDA TRAJ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - MARIA GORETI HERCULAN...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- INSTITUTO VISÃO: 2° Quinz. Maio > PIX: 35.128.190/...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - HELENA SOARES...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: ROB SOL INDUSTRIAL LTDA - Parcela 3/18 > P...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - SUNAMITA R S ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- RESCISÃO: Kethelyn Miranda - Acordo 4/4 > PIX: PIX...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - MARLI FERREIRA FRANCO...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - SILVIO GERALD...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - NIVALDA RABEL...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SALÁRIO: Eliana Savoia > PIX: (11) 9 4714-9764 (ce...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- COMISSÃO: Felipe Roger Miranda (ref. OS entregue ~...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Compra de Armações receitoário - 02 óculos de segu...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: ACERTO FRANQUIA > Parcela 15/40 > PIX: 39....
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- HONORÁRIOS: Bruna Giovanna Cardoso > Parcela 2/8 >...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CARLOS APARECIDO DE S...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - MATEUS DA SILVA CAVAL...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - KAIO GABRIEL ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SUPERMERCADO ASSAÍ: Compra de utencilios para as l...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX ENVIADO - SANTA HELENA DOCES E P...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
