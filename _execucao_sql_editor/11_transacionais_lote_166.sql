...
-- Conciliação - PIX RECEBIDO - CARLOS EDUARDO CARVAL...
DO $$
DECLARE
...
-- Conciliação - PIX RECEBIDO - SANDRA DIAS ANDRADE S...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - JAIME RODRIGU...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - 56524988 YASMIN CRIST...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - PEDRO EDUARDO...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - DIEGO HUMBERTO PIMENT...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - ADENILSON SOA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: ACERTO FRANQUIA > Parcela 31/40 > PIX: 39....
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: ACERTO FRANQUIA > Parcela 1/54 > PIX: 39.4...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - ALEXSANDRO DA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ACORDO: VISION PRIME - Parcela 16/49 > PIX: 47.392...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - VALDIR ALVES ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CAIQUE TAVARES DA SIL...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- ARMARINHOS FERNANDO: Grampos > Nicolas Coutinho da...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SALÁRIO: Melissa Ketelyn Sanches de Oliveira (divu...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - PABLO ROMEU ROCHA FER...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - ANDERSON ALMEIDA RIBE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- VALE TRANSPORTE: Melissa Ketelyn Sanches de Olivei...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - ISABEL DE OLI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - LEONILDA MARTINS MORE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- COMPRA DE ITENS PARA LOJA: Cartão Fidelidade + Tag...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - FRANCISCO DAL...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- RESCISÃO: Eliana Savoia - Acordo 2/6 > PIX: 276.52...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'RESCISÃO: Eliana Savoia - Acordo 2/6 > PIX: 276.526.448-10 (CPF)',
        'RESCISÃO: Eliana Savoia - Acordo 2/6 > PIX: 276.526.448-10 (CPF)',
        '2024-10-01'::date,
        1000.0,
        1,
...
-- Conciliação - PIX RECEBIDO - CARMEM LUCIA DE L PER...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- RESCISÃO: Érika de Oliveira Gomes - Acordo 8/8 > P...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - CARTÃO DE DÉBITO - LUIZ ROBERTO REST...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - CARTÃO DE DÉBITO - AUTO POSTO RAGUEB...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - QR CODE PIX RECEBIDO - ADRIANA APARE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- INSTITUTO VISÃO:  2° Quinz. Setembro > PIX: 35.128...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
