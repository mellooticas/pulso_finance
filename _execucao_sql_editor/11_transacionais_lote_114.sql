...
-- Conciliação - PIX RECEBIDO - CP :18236120-ESTER BA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :00000000-LUIS ANT...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-ELISANGE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-LEIDIANN...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :90400888-ELIAS ZE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60746948-VANDA PI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60701190-FABIO DA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60701190-ARTHUR M...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60746948-JENNIFER...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-HELEN DA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-ROSIANE ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :08561701-PEDRO CA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :08561701-VALDIREN...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- MERCADINHO VALE DAS COMPRAS LTDA: Itens para a loj...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :00000000-ODETE MO...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-CÍCERO M...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60746948-MARIA AU...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SALÁRIO: Marcelo Narcisio Santos (divulgador, ref....
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- 30/06/2023 - BRASLAB PRODUTOS OTICOS EIRELI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :00360305-MARIA AU...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SALÁRIO: Marcelo Narciso Santos (divulgador, ref. ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-MONIQUE ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60701190-GILVANA ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX - RECEBIDO31/07 21:22 0009725720...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60701190-MATHEUS ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-YASMIN D...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SUPERMERCADO NAGUMO: Multiuso, detergente e água s...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :08561701-JESUELTO...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-ANA MARI...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :10573521-GILVANA ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SUMUP - PIX RECEBIDO - CP :08561701-GILBERTO DE CA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
BEGIN
    ...
    INSERT INTO lancamentos (
        ...
        plano_id_ref,
        NULL,
        'SUMUP - PIX RECEBIDO - CP :08561701-GILBERTO DE CAMARGO SILVA JUNIOR',
        'SUMUP - PIX RECEBIDO - CP :08561701-GILBERTO DE CAMARGO SILVA JUNIOR',
        '2023-07-31'::date,
        1229.81,
        1,
...
-- Conciliação - PIX RECEBIDO - CP :90400888-KATIA VA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-UBALDO F...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- pix - COMPRA CARTAO - NO ESTABELECIMENTO JAPALLU R...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60746948-LUCELIA ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- RESCISÃO: Lara Silva Wingeter de Souza - Acordo 3/...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :24313102-VIVIANE ...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :90400888-ELINEIDE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Troca de braços da cadeira giratória - Reginaldo D...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- CORANTE TINTAS LOJA MATRIZ:  Aplicador de massa gr...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- SALÁRIO: Wellen Santos Silva (divulgadora, ref. os...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- CASA ROLLU PAPELARIA E BAZAR LTDA: Saquinho plásti...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :37880206-DNA ANIM...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :60701190-JONATHAS...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- VALE ALIMENTAÇÃO/REFEIÇÃO (provisório - Rio Pequen...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Devolução de PIx, pago em duplicidade: Eliana Savo...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :10573521-GIOVANNA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :10573521-PEDRO CA...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :18236120-PATIENCE...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
...
-- Conciliação - PIX RECEBIDO - CP :09554480-FLAVIA S...
DO $$
DECLARE
    novo_lanc_id UUID := gen_random_uuid();
    plano_id_ref UUID;
