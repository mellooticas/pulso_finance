...
export default function ContasReceberPage() {
  ...
  // Hooks de dados - filtrando apenas receitas
  const { data: lojas } = useLojas()
  const { data: contasReceber, isLoading } = useLancamentos({
    data_inicio: periodo.inicio,
    data_fim: periodo.fim,
    tipo: 'receita', // Apenas receitas
    ...filtros,
  ...
  const contasVencidas = contasReceber?.filter(conta => {
    ...
    return conta.status !== 'recebido' && conta.status !== 'pago' && vencimento && vencimento < hoje
  }) || []

  const contasVencendoSemana = contasReceber?.filter(conta => {
    const hoje = new Date()
    const semanaQueVem = new Date(hoje.getTime() + 7 * 24 * 60 * 60 * 1000)
    const vencimento = conta.data_vencimento ? new Date(conta.data_vencimento) : null
    return conta.status !== 'recebido' && conta.status !== 'pago' && vencimento && vencimento >= hoje && vencimento <= semanaQueVem
  }) || []
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-gray-600">
            Gerencie todas as contas e valores a receber
          </p>
        </div>
          ...
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-green-600">
                {formatCurrency(stats?.totalReceitas || 0)}
              </p>
            </div>
          </div>
        </Card>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-2">
                          {conta.status !== 'recebido' && conta.status !== 'pago' && (
                            <button className="text-green-600 hover:text-green-900" title="Marcar como recebido">
          ...
          <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div>
              <p className="text-sm text-gray-700">
                ...
                até{' '}
                <span className="font-medium">
                  {Math.min(paginacao.pagina * paginacao.itensPorPagina, contasReceber?.length || 0)}
                </span>{' '}
                de{' '}
                <span className="font-medium">{contasReceber?.length || 0}</span>{' '}
                resultados
              </p>
            </div>
            <div>
